...
async def create_table_report(id):
    ...
    worksheet.update_cell(2, column, lines[0])
    if len(lines) > 1:
        for k in range(1, len(lines) - 1):
            for i in range(len(all_values)):
                if str(lines[k].split()[0]) == (all_values[i][0]):
                    worksheet.update_cell(i + 1, column, float(lines[k].split()[1].strip()))
                    break
        for i in range(len(all_values)):
            if all_values[i][5] == "Рабочие":
                k = i + 1
                for installer in sorted(lines[-1].split(',')):
                    worksheet.update_cell(k, column, installer)
                    k += 1
...
async def fill_zeros(id):
    ...
    with open(f'report_info/{id}.txt', 'r', encoding='utf-8') as file:
        date = file.readlines()
    worksheet.update_cell(2, column, date[0].strip())
